...
* **MCP Server** — the [AI integration](../integrations/ai-integration-mcp.md) endpoint is probed on its own track, without blocking the other checks. It scales to zero, so a first probe often finds it asleep: the card then says *Starting instance…* with the attempt count and keeps re-checking for about a minute before it reports a problem.
