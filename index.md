...
* [Autopilot Device Preparation](getting-started/autopilot-device-preparation.md) - Device Preparation monitoring: MSI line-of-business delivery in the first phase, enrollment-time group scoping, Corporate Identifier validation.
...
* [Data Flows & External Services](trust/data-flows.md) - Every outbound connection the service makes, grouped by what actually happens to customer data; the DPA remains authoritative for the contractual side.
...
* This bundle is also the corpus behind the MCP server's `search_docs` tool. The corpus is built into the MCP container image, so a change here reaches AI clients only after the MCP server is redeployed. `https://mcp.autopilotmonitor.com/health` reports the commit of the bundle the running server was built from — compare it against this repository's `main` to see whether a redeploy is outstanding.
* The Trust & Security pages state what the service commits to. Changes to hosting region or topology, storage redundancy, encryption, retention caps, the authorization or delegation model, external services and data flows, notification providers, agent build and signing, backup scope, or a default that governs what is collected (geolocation, diagnostics upload destination, trace events) update [trust/security-faq.md](trust/security-faq.md) and [trust/data-flows.md](trust/data-flows.md) in the same change, including the `timestamp` frontmatter and the visible "Last reviewed" line. Customer-visible changes there are also announced in [troubleshooting/service-announcements.md](troubleshooting/service-announcements.md). Phrase these pages durably rather than exactly — "short-lived, measured in minutes" instead of a specific TTL — so routine tuning does not invalidate a published statement.
