...
* **Time attribution — where enrollment time actually goes** — Finished enrollments now carry a breakdown under the phase timeline: device preparation, apps (ESP), identity & Hello, user ESP, desktop handoff, and an honest unattributed remainder, plus reboot time and the ESP-blocking apps on the critical path. Fleet Health shows the same split as medians per enrollment class, with the apps that cost the most and an *"up to"* estimate of what taking one off the critical path would save. See [Fleet Health](../portal-guide/fleet-health.md#time-attribution) and [Session Details](../portal-guide/session-details-and-diagnosis.md#time-attribution).
* **First-time-right — wipe-and-retry is now visible** — Enrollment attempts of the same device are grouped into a [device journey](../concepts/sessions-and-statuses.md#device-journeys--attempts), so you can see how many devices needed a second or third run: a rate with a weekly trend, a histogram of attempts until success, and a list of the repeat devices with their last failure reason. See [Fleet Health](../portal-guide/fleet-health.md#first-time-right).
...
* **Rule regression detection** — When an analyze rule starts firing far more often than usual — statistically separated from its own 28-day baseline, not just a noisy week — the rule card shows a **↑ Regression** badge and tenant admins get a bell notification with the numbers and, where one exists, the OS build, model, or agent version the affected sessions concentrate on. Rule cards also gained a 30-day fire sparkline. See [Analyze Rules](../rules/analyze-rules/README.md#regression-detection).
* **App durations count real installs only** — Apps that Intune reported as *skipped* (e.g. an update policy that didn't apply) no longer count as 0-second installs. Average and maximum install duration and the slowest-apps ranking are computed over measured installs; skipped counts are shown separately.
* **Install rows say where they come from** — Rows in Install Progress that are not plain Intune app installs now carry a **Click-to-Run** or **RealmJoin** pill, so a Win32 app and the installer it triggers no longer look like a duplicate.
* **Devices with an incompatible TPM are surfaced to the tenant** — Devices whose TPM cannot sign with RSA-PSS (Windows 11 25H2+ withholds the client certificate, so the agent can never authenticate and the enrollment is never monitored) now appear in the Hardware Whitelist section with remediation guidance, plus a one-time bell notification.
