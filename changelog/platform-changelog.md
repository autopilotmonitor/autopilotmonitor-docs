...
* **Progress Portal for every enrollment scenario** — The self-service status page now follows Device Preparation, self-deploying, and pre-provisioned enrollments with the right steps and live app progress instead of the Autopilot ESP phases only. See [Progress Portal](../portal-guide/progress-portal.md).
