...
* **App install times now measure the actual install attempt** — The Intune Management Extension processes the app list in several passes: an early evaluation pass can touch an app long before its real install runs later in the queue. App durations used to span from the first observation to the final outcome — so a 23-second app late in the queue could show an hour. They now measure the **final install attempt**, with a small ×2 marker on apps processed in multiple passes; download time is reported separately. The *Slowest Apps* ranking, per-app averages/percentiles, and the session **time attribution** (which now counts an app's active time across its passes, never the idle gap between them) all follow the new definition. Values recorded before this change keep the old span measurement, so averages and charts step down visibly around the changeover — that is the correction, not a real fleet improvement. Version-regression alerts compare only values measured under the new definition.
* **Analyze rules can now run during the enrollment** — Rules always ran once, at enrollment end; a stuck-but-active session was never analyzed and WhiteGlove findings arrived after the technician had left. Rules can now declare additional **evaluation triggers**: at the WhiteGlove seal, or whenever specific events arrive mid-enrollment. Findings raised early appear live as **Preliminary** on the session page and are confirmed or resolved by the final analysis; notifications fire at most once per finding, and interim runs never fail a session or skew statistics. The rule editor gained an **Evaluation Triggers** section; the new built-in rule **ANALYZE-ID-004** uses it to flag hybrid-join devices whose user sign-in never registers with Entra — while the device is still looping. Also new: **ANALYZE-APP-015** names required apps that block enrollment completion without ever starting to install. Most built-in device-phase rules (30 of 43) now also evaluate at the WhiteGlove seal, and six additionally fire the moment their triggering event arrives (failed app installs, failed Windows Updates, TPM attestation errors). See [Concepts → Evaluation triggers](../rules/analyze-rules/concepts.md#evaluation-triggers-when-a-rule-runs).
* **App version duration regression alerts** — When a newly rolled-out app version's median install duration rises sharply above the previous version's (at least twice as long and 5+ minutes more, over measured installs only), the app's detail page shows a regression banner and tenant admins get a one-time notification. Version charts now also include **median install duration by version**. See [Software](../portal-guide/software-inventory-and-vulnerabilities.md#per-app-deep-dive).
* **Fleet context on analysis findings** — Each finding on a session now says in how many enrollments the rule fired over the last 14 days, and **View sessions** opens the dashboard filtered to exactly those enrollments — one click from "this device has a problem" to "how widespread is it?". See [Session Details](../portal-guide/session-details-and-diagnosis.md#session-detail).
* **Gather rules: guardrail-aware enabling and clearer attribution** — WMI gather rules can now select specific properties of an allow-listed class (e.g. `SELECT BatteryStatus FROM Win32_Battery` — ideal with emit mode *On change*; requires the latest agent). A custom rule whose target is not on the collection allow-lists can no longer be enabled: new ones are created disabled, an edit that makes the target invalid disables the rule, and an already-enabled one shows a **Blocked on devices** badge — with a hint to [request an allow-list addition](https://github.com/okieselbach/AutopilotMonitor/issues). New custom rules also record the creating admin as **Author** instead of "Autopilot Monitor".
* **Windows 365 Cloud PC support** — Cloud PC first-connect enrollments can now be monitored. Cloud PCs are never Autopilot-registered, so device validation used to reject them; a new opt-in **Windows 365 Cloud PC Validation** check (backed by a scoped Graph add-on) admits them instead — enable it in Settings and deploy the latest bootstrapper script. Rejected Cloud PCs in *Devices Not Registered* now carry a Cloud PC badge with a hint pointing at the new toggle.
...
* **Incompatible-TPM devices are surfaced** — Devices whose TPM cannot sign with RSA-PSS (Windows 11 25H2+ withholds their client certificate, so they can never be monitored) now appear in the Hardware Whitelist with remediation guidance, plus a one-time notification.
...
* **Fixed: portal could freeze on navigation** — Background page prefetching could overload the hosting tier and leave tabs on a spinner for minutes; prefetching is now disabled and the hosting plan upgraded.
