...
| **Autopilot Device Preparation** | Supported, including [device association](autopilot-device-preparation.md#device-association) as a validation method. The recommended agent delivery is the MSI line-of-business app; the platform script works too but starts later. See [Autopilot Device Preparation](autopilot-device-preparation.md). |
