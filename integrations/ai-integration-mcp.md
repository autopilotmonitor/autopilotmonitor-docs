...
The client receives its refresh tokens only encrypted and bound to the registration; only the MCP server can open them. Deleting a registration therefore ends the client's access within about a minute: no new sign-ins and no token refreshes, including refresh tokens copied out of the client's storage. An access token already issued stays valid until it expires, typically after about an hour. Every registration and deletion is written to your audit log.
...
Tokens are issued by Microsoft Entra ID and follow its lifetimes: an access token typically lasts about an hour, and a refresh token renews itself while it is in use. To end a person's MCP access, remove their role or block the account; this takes effect on their next request within about two minutes, whatever token they hold. Revoking the user's sessions in Entra also invalidates the refresh tokens already issued; a password change by the user alone does not.
