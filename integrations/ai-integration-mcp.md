...
**Delegated (MSP) administrators:** a request into a tenant you manage counts against **that tenant's** organization budget and follows **its** plan — a Community customer is read with Community windows even from a Pro tenant. Your own per-account budget still comes from your home tenant. When a managed tenant's budget is exhausted, the assistant is told which tenant it was, and your other managed tenants stay available; the `get_fleet_overview` tool, which gives a bounded overview across all your managed tenants, skips exhausted tenants and lists them instead of failing. The customer's tenant admins see your requests on their MCP Usage page, marked as delegated.
