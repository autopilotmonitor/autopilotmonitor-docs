...
| **ANALYZE-OFFICE-001** · Microsoft 365 Apps Install Failed | warning | The Office Click-to-Run installation reported an error code — a failure the Intune app status can *hide*, because Intune marks the app done when its package or configuration step ends while Click-to-Run keeps installing in the background. See [Microsoft 365 Apps appears twice](../../troubleshooting/microsoft-365-apps-shown-twice.md). |
...
| **ANALYZE-ESP-004** · ESP Gave Up on a Blocking App with 'Continue Anyway' (Soft Failure) | warning | The ESP gave up while a blocking app was still installing — no error code, the app simply never finished inside the ESP window — and the profile allows *Continue anyway*, so the user most likely reached the desktop on a working, not fully provisioned device. Stays silent when the failed app reported a real error code; that case belongs to the rule for the code. A cluster of these usually means one slow blocking app is holding the fleet at the ESP limit. |
...
| **ANALYZE-ID-004** · Hybrid Sign-In Never Establishes Entra User Affinity | high | On a hybrid-joined device the user reached the desktop, but the Intune Management Extension never acquired that user's Entra token (no Primary Refresh Token) — the agent reports *user affinity pending* with the token error codes, repeatedly across reboots — so user-targeted apps, a user ESP and Windows Hello never settle. Points at Entra Connect sync scope, UPN mismatch, or missing domain-controller line of sight at logon. Also evaluated *while the enrollment is still running* (see [evaluation triggers](concepts.md#evaluation-triggers-when-a-rule-runs)). See [Hybrid Join: stuck in Account Setup](../../troubleshooting/hybrid-join-stuck-in-account-setup.md). |
